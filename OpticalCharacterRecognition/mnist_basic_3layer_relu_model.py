...
weights_l1 = tf.Variable(tf.truncated_normal([784,1200], stddev=0.01))
bias_l1 = tf.Variable(tf.zeros([1200]))

weights_l2 = tf.Variable(tf.truncated_normal([1200,1200], stddev=0.01))
bias_l2 = tf.Variable(tf.zeros([1200]))

weights_l3 = tf.Variable(tf.truncated_normal([1200,10], stddev=0.01))
bias_l3 = tf.Variable(tf.zeros([10]))

#The Model:
layer1_output = tf.nn.relu(tf.matmul(inputs,weights_l1)+bias_l1)
layer2_output = tf.nn.relu(tf.matmul(layer1_output,weights_l2)+bias_l2)
model_output = tf.nn.sigmoid(tf.matmul(layer2_output,weights_l3)+bias_l3)
